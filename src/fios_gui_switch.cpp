...
// The save dialog crashes on Switch if this FileList is a private member of SaveLoadWindow
// or any optimization other than -O0 is used.
static FileList _fios_items;
	...
	virtual void OnInvalidateData(int data = 0, bool gui_scope = true)
	{
		switch (data) {
			case SLIWD_RESCAN_FILES:
				...
				_fios_items.BuildFileList(this->abstract_filetype, this->fop);
			...
			case SLIWD_FILTER_CHANGES:
				/* Filter changes */
				this->fios_items_shown.Resize(_fios_items.Length());
				...
				for (uint i = 0; i < _fios_items.Length(); i++) {
					if (this->string_filter.IsEmpty()) {
						...
						this->string_filter.AddLine(_fios_items[i].title);
						...
						if (&(_fios_items[i]) == this->selected && this->fios_items_shown[i] == false) {
